...
const SVG3 = () => (
    <g>
        <path clipRule="evenodd" d="m311.033 333.001c0-20.491 2.941-26.562 32.531-65.373 54.987-70.488 2.787-177.518-87.533-176.626-90.312-.878-142.527 106.131-87.528 176.627 29.589 38.81 32.529 44.881 32.529 65.372z" fill="#fff69b" fillRule="evenodd" />
        <g id="process_outline">
            ...
            <path fill="#333" d="m256.033 151.001c-24.04 0-46.641 9.362-63.64 26.361-3.905 3.905-3.905 10.237 0 14.142s10.237 3.905 14.142 0c26.055-27.156 72.94-27.155 98.994.001 1.953 1.952 4.512 2.928 7.071 2.928 8.68.154 13.409-10.92 7.071-17.071-16.997-16.999-39.598-26.361-63.638-26.361z" />
            <path fill="#333" d="m284.317 222.716c8.692.152 13.402-10.922 7.071-17.071-18.611-19.395-52.1-19.396-70.711.001-9.031 9.679 4.452 23.18 14.143 14.142 5.666-5.666 13.199-8.787 21.213-8.787s15.547 3.121 21.213 8.787c1.952 1.952 4.512 2.928 7.071 2.928z" />
            <path fill="#333" d="m246.029 241.001c0 5.526 4.482 10.005 10.008 10 13.25-.491 13.239-19.514-.008-20-5.523 0-10 4.477-10 10z" />
...
const aboutData = [
    { heading: 'Pixel Perfect', bg_frame: '/images/round1.png', svg: <SVG1 />, desc: 'Designs that translate to code with precision — crisp spacing, consistent typography, and responsive layouts across every breakpoint.' },
    { heading: 'High Quality', bg_frame: '/images/round2.png', svg: <SVG2 />, desc: 'Clean architecture, typed APIs, and thoughtful testing so the product stays fast and maintainable as it grows.' },
    { heading: 'Awesome Idea', bg_frame: '/images/round3.png', svg: <SVG3 />, desc: 'From whiteboard to deploy: I help shape the idea, choose the right stack, and ship it end-to-end.' },
];

const Section2 = () => {
    const [ref, visible] = useReveal();
    return (
        <div className="section2_content min-h-[10vh] border-0 py-[4rem] md:py-[6rem] lg:py-[8rem]">
            <div
                ref={ref}
                className={`container flex flex-col gap-8 lg:gap-4 lg:flex-row justify-between items-center lg:items-stretch px-4 ${visible ? 'reveal reveal-up' : 'reveal'}`}
            >
                {aboutData.map((item, index) => (
                    <div
                        key={index}
                        className="service-card w-full max-w-sm lg:w-1/3 text-center"
                        style={{ transitionDelay: `${index * 120}ms` }}
                    >
                        <span className="dizme_tm_process">
                            <img className="brush" src={item.bg_frame} alt="" />
                            <svg
                        ...
                        <h3 className="text-[24px] md:text-[28px] font-medium mb-0 lg:mb-3 color-orange mt-2">
                            {item.heading}
                        </h3>
                        <p className="border-0 py-2 rounded-md text-[0.95rem] md:text-[1rem]">{item.desc}</p>
                    </div>
                ))}
            </div>
        </div>
